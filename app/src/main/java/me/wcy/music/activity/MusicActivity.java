...
public class MusicActivity extends BaseActivity implements View.OnClickListener, OnPlayerEventListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupView();//设置NavigationView和初始化ViewPager的适配器
        updateWeather();//可不看
        registerReceiver();//耳机线控
        onChange(getPlayService().getPlayingMusic());//切换歌曲
        UpdateUtils.checkUpdate(this);//更新版本，可不看
        parseIntent();//暂时不知道干嘛的
    }
    ...
    @Override
    protected void setListener() {//onStart里调用
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.iv_menu://不看
                drawerLayout.openDrawer(GravityCompat.START);
                break;
            case R.id.iv_search://不看
