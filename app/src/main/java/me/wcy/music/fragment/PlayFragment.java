...
public class PlayFragment extends BaseFragment implements View.OnClickListener,
    ...
    protected void init() {
        initSystemBar();//不明白...
        initViewPager();//初始化ViewPager，用于切换专辑封面和全屏歌词界面的切换。
        ilIndicator.create(mViewPagerContent.size());
        initPlayMode();//从sharepreference中获取保存的播放模式
    ...
    private void initViewPager() {
        ...
        mAlbumCoverView.initNeedle(getPlayService().isPlaying());//自定义view的内容
        initVolume();//音量设置
    ...
    private void initPlayMode() {
        ...
        ivMode.setImageLevel(mode);//设置LevelListDrawable
    }
    ...
    public void onPublish(int progress) {
        ...
        //更新当前播放时间
        if (progress - mLastProgress >= 1000) {//时间大于1秒才进行操作
    ...
    private BroadcastReceiver mVolumeReceiver = new BroadcastReceiver() {//音量改变的广播接收者
        @Override
